...
while viewer.is_running():
    ...
    if result.hand_landmarks:
        ...
        thumb_bend = np.clip(
            np.interp(thumb_dist, [0.05, 0.30], [0, 1.5]),
            0,
            1.5
        )
    ...
    if ser.in_waiting:
        try:
            ...
            # Map sensor range
            bend = np.interp(val, [520, 610], [0, 2])
            bend = np.clip(bend, 0, 2)

            print("VAL:", val, "BEND:", bend)  # 👈 debug

            # Override index finger
            ctrl[8:11] = bend
